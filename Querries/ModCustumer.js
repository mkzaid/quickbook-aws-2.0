...
const ModCustumerQuerryGenerator = (body)=>{
    let xmlQuery = ``;
    const {
      ListID,
      EditSequence,
    ...
    if (!ListID || !EditSequence) {
      return res.status(400).json({ error: "ListID and EditSequence are required" });
    }
    xmlQuery = `<CustomerModRq>
        <CustomerMod>
...
if (AccountNumber) xmlQuery += `<AccountNumber>${AccountNumber}</AccountNumber>`;
...
if (JobStartDate)
  xmlQuery += `<JobStartDate>${JobStartDate}</JobStartDate>`;
...
if (JobTypeRef) {
  ...
  ${
    JobTypeRef.FullName
      ? `<FullName>${JobTypeRef.FullName}</FullName>`
      : ""
  }
</JobTypeRef>`;
}
...
// Close the CustomerMod and CustomerModRq tags
xmlQuery += `</CustomerMod>
  </CustomerModRq>`;
